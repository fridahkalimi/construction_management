...
jobwindow = importlib.import_module('jobs')
memberwindow = importlib.import_module('members')
issuejobwindow = importlib.import_module('issuejob')
...
class System:
    def __init__(self,master):
        ...
        def showsummary(self):
            ...
            book_instock_counter = cur.execute("SELECT COUNT(job_id) FROM jobs WHERE job_status").fetchall()
            member_counter = cur.execute("SELECT COUNT(member_id) FROM member").fetchall()
            issued_counter = cur.execute("SELECT COUNT(job_id) FROM jobs WHERE job_status=1").fetchall()
            self.lbl_job_count.config(text="Avalable: "+str(job_instock_counter[0][0]))
        ...
        def showjobs(self):
            ...
            for job in jobs:
                    self.management_box.insert(counter,str(job[0])+'|'+job[1])
            counter+= 1
        ...
        def jobinfo(evt):
                ...
                job = cur.execute("SELECT * FROM jobs WHERE job_id=?",(id))
                job_info = job.fetchall()
                self.list_details.insert(0, 'Vacancy name:'+job_info[0][1])
                self.list_details.insert(1, 'company name:'+job_info[0][2])
                self.list_details.insert(2, 'slots available:'+str(job_info[0][3]))

                if job_info[0][4] == 0:
                    self.list_details.insert(3,'Status: Available')
                else:
                    self.list_details.insert(3,'Status: Not Available')
                    self.management_box.bind('<<ListboxSelect>>',jobinfo)
        ...
        self.btn_add_job = Button(top_frame, text="Add job vacancy", font="arial 12 bold",padx=10,command=self.new_job)
        self.btn_add_job.pack(side=LEFT)

        self.btn_issue_job= Button(top_frame, text="Issue job", font="arial 12 bold",padx=10, command = self.issue_job)
        self.btn_issue_job.pack(side=LEFT)
        ...
        #summary
        self.lbl_job_count = Label(self.tab2, text='', pady=20, font='verdana 14 bold')
        self.lbl_job_count.grid(row=0)
        ...
        self.label_search = Label(searchbar, text='Search job', font='arial 12 bold')
        ...
        list_bar = LabelFrame(right_frame, width=280, height=200, text='Job List', bg="#fff")
        ...
        rbtn_all_jobs = Radiobutton(list_bar, text='Sort all jobs', var=self.list_choice,value=1)
        rbtn_all_jobs.grid(row=1,column=0)
        rbtn_instock = Radiobutton(list_bar, text='jobs available', var=self.list_choice,value=2)
        rbtn_instock.grid(row=1,column=1)
        rbtn_issued_jobs = Radiobutton(list_bar, text='jobs issued', var=self.list_choice,value=3)
        rbtn_issued_jobs.grid(row=1,column=2)

        btn_show_jobs = Button(list_bar, text='Show jobs', font='arial 12 bold',command = self.searchsort)
        btn_show_jobs.grid(row=1,column=3,padx=40,pady=10)

        welcome_image = Frame(right_frame, width=300, height=400)
        welcome_image.pack(fill=BOTH)
        self.welcome_main_image = PhotoImage(file='images/app.png')
    ...
    def searchsort(self):
        ...
        if value == 1:
            ...
            query = "SELECT * FROM jobsWHERE job_status = 0"

        else:
            query = "SELECT * FROM jobs WHERE job_status = 1"

        self.management_box.delete(0,END)
        counter = 0
        searchquery = cur.execute(query).fetchall()
        for job in searchquery:
            self.management_box.insert(counter, str(job[0])+'-'+str(job[1]))
            counter += 1

    def issue_job(self):

        add = issuejobwindow.Issuejob()

    def new_job(self):

        add = jobwindow.Storejob()
    ...
    def search(self):
        ...
        searchquery = cur.execute("SELECT * FROM jobs WHERE job_name LIKE ?",('%'+value+'%',)).fetchall()
        self.management_box.delete(0,END)
        counter = 0
        for job in searchquery:
            self.management_box.insert(counter, str(job[0])+'-'+str(job[1]))
...
def base():
    ...
    base_window.title("Construction Management System")
